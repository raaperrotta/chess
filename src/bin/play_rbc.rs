...
fn main() {
    let mut white = chess::AttackerPlayer::new(chess::Color::White);
    let mut black = chess::MhtPlayer::new();
    let result = chess::play_rbc(&mut white, &mut black);
    match result {
        chess::GameOverReason::KingCapture(color) => println!("{:?} wins by king capture!", color),
        chess::GameOverReason::IllegalMove(color) => {
            println!("{:?} disqualified for requesting an illegal move!", color)
        }
